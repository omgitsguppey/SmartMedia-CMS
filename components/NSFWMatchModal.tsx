    ...
    <div className="fixed inset-0 z-[80] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="w-full max-w-4xl flex flex-col items-center space-y-8">
        <div className="text-center space-y-2">
           <h2 className="text-3xl font-bold bg-gradient-to-r from-ios-danger to-orange-500 bg-clip-text text-transparent flex items-center justify-center gap-3">
           ...
           <p className="text-zinc-400">Verify content flagged as potentially unsafe.</p>
        </div>
        ...
        {currentCandidate ? (
           <div className="w-full animate-in zoom-in-95 duration-300 flex flex-col items-center">
             ...
             <div className="relative w-full max-w-2xl h-[400px] bg-ios-surface2 rounded-2xl overflow-hidden border border-ios-danger/30 shadow-2xl shadow-ios-danger/10 mb-8 group">
                ...
                {currentCandidate.type === MediaType.AUDIO && (
                  <div className="w-full h-full flex items-center justify-center bg-zinc-900">
                     <svg className="w-24 h-24 text-zinc-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
                  </div>
                )}
                
                <div className="absolute top-4 left-4 px-3 py-1.5 bg-ios-danger text-white text-xs font-bold rounded-lg shadow-lg uppercase tracking-wide">
                  Possible NSFW
                </div>
             </div>

             <div className="flex flex-col items-center gap-6 w-full max-w-lg">
                <p className="text-zinc-300 text-lg font-medium">Does this contain unsafe content?</p>
                
                <div className="flex w-full gap-4">
                  <button 
                    onClick={() => onReview(currentCandidate.id, false)}
                    className="flex-1 py-4 rounded-xl bg-ios-success/10 text-ios-success border border-ios-success/20 hover:bg-ios-success hover:text-white font-bold transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 py-4 rounded-xl bg-ios-danger/10 text-ios-danger border border-ios-danger/20 hover:bg-ios-danger hover:text-white font-bold transition-all flex items-center justify-center gap-2"
                  >
           ...
           <div className="text-center space-y-6 max-w-md animate-in slide-in-from-bottom-4">
              ...
              <div>
                 ...
                 <p className="text-zinc-400 mt-2">There are no more flagged items to review.</p>
              </div>
